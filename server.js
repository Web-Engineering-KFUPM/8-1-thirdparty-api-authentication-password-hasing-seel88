...
app.get("/weather", async (req, res) => {
  try {
    ...
    return res.json({
      city,
      temp: data.temperature || current.temp_C || current.temp_F || null,
      description:
        data.description || current.weatherDesc?.[0]?.value || null,
      wind: data.wind || current.windspeedKmph || current.winddir16Point || null,
      raw: data,
    });
  } catch (err) {
